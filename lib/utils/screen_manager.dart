...
double getWp(double dp) => _ScreenManager.instance.setWidth(dp);
double getHp(double dp) => _ScreenManager.instance.setHeight(dp);

double getSp(double sp) => _ScreenManager.instance.setSp(sp);
Size getSize(double size) => Size.fromWidth(getWp(size));

void initScreen({double width = 750, double height = 1334}) {
  _ScreenManager.instance = _ScreenManager(
    width: width,
    height: height,
  )..init();
}
...
class _ScreenManager {
  static _ScreenManager instance = new _ScreenManager();
  ...
  static _ScreenManager getInstance() {
    return instance;
  }
