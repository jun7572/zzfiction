...
class ReadController extends SuperController {
  ...
  int index11 = 0;
  //屏幕居中点击事件
  centerlickEvent(TapDownDetails e) {
    int length = 80;
  ...
  loadChapter({int index}) async {
    ...
    if (fs.currentFictionChapter.content == null||fs.currentFictionChapter.content.isEmpty||SearchEngine.noData==fs.currentFictionChapter.content) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      pc.nextPage(
          duration: Duration(milliseconds: 200), curve: ElasticInOutCurve());
    });
  }
  ...
  onPageViewChange(page) async {
    // FictionRepository fs = Get.find<FictionRepository>();
    // var swipingRight = page > pc.page;
    //
    // if (swipingRight) {
    //   fs.currentFictionChapter.index++;
    // }else{
    //   fs.currentFictionChapter.index--;
    // }
    // update();
